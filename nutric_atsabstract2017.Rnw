...
variables rather than summary variables. Most continuous covariates (noted below) were allowed to
have a nonlinear relationship with the outcomes using restricted cubic splines; other continuous
covariates were forced to have a linear relationship, due both to limited sample size and to lack of
variability in those covariates (eg, over 75\% of baseline ADL values were 0).
...
\item In-hospital characteristics
  \begin{itemize}
  \item Mean modified daily SOFA in ICU (nonlinear) (\emph{mortality: daily modified SOFA})

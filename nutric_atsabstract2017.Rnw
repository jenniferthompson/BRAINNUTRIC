...
data are reported for each model in section \ref{section:missingness}.
...
rms.results.data <- function(outcome = c('rbans', 'trailb', 'ltmort', 'dchome')){
  if(outcome == 'dchome'){
    mod.il6 <- get(paste0(outcome, '.il6'))
    mod.noil6 <- get(paste0(outcome, '.noil6'))
...
results.table <- function(outcome = c('adl', 'faq', 'rbans', 'trailb', 'ltmort', 'dchome')){
  if(outcome %in% c('adl', 'faq')){
...
for, respectively, hospital survivors (used for discharge location and long-term mortality models) and long-term cognitive and disability cohorts.

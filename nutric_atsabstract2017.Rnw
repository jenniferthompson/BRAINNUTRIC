...
due to the potential for immortal time bias with this outcome). All continuous covariates are
allowed to have a nonlinear relationship with each outcome, using restricted cubic splines.
...
\item In-hosptial characteristics
  \begin{itemize}
  \item Mean modified daily SOFA in ICU (\emph{in-hospital death: daily modified SOFA})
  \item Duration of severe sepsis (\emph{in-hospital death: daily severe sepsis})
  \item Duration of delirium (\emph{in-hospital death: daily delirium})
  \item Duration of coma (\emph{in-hospital death: daily coma})
  \item Duration of mechanical ventilation (\emph{in-hospital death: daily MV})
  \end{itemize}
\end{itemize}

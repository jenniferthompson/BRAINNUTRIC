...
create.rms.preddata <- function(outcome = c('rbans', 'trailb', 'ltmort', 'dchome')){
  ...
  if(outcome == 'dchome'){
    ...
    pred.il6$time <- pred.noil6$time <- 1
...
yaxis.labels <- data.frame(outcome = c('adl', 'faq', 'rbans', 'trailb', 'ltmort', 'dchome'),
                           ylabel = c('Katz ADL Score', 'FAQ Score', 'RBANS Global Score',
                                      'Trails B Score', 'Hazard of Death',
                                      'Probability of Discharge to Non-Home'))

## Plotting function
create.nutric.plot <- function(outcome = c('adl', 'faq', 'rbans', 'trailb', 'ltmort', 'dchome')){
  ...
  if(outcome %in% c('adl', 'faq')){
    ...
    ymin.var <- 'count.lcl'
    ymax.var <- 'count.ucl'
  } else if(outcome %in% c('rbans', 'trailb', 'ltmort', 'dchome')){
...
rms.results.data <- function(outcome = c('rbans', 'trailb', 'ltmort')){
  mod.il6.3 <- get(paste0(outcome, '.il6.3'))
  mod.noil6.3 <- get(paste0(outcome, '.noil6.3'))
  mod.il6.12 <- get(paste0(outcome, '.il6.12'))
  mod.noil6.12 <- get(paste0(outcome, '.noil6.12'))
  ...
  if(outcome %in% c('ltmort')){
    exp.cols <- c(4, 6, 7)
    diffci.il6.3[exp.cols] <- exp(diffci.il6.3[exp.cols])
    diffci.noil6.3[exp.cols] <- exp(diffci.noil6.3[exp.cols])
    diffci.il6.12[exp.cols] <- exp(diffci.il6.12[exp.cols])
    diffci.noil6.12[exp.cols] <- exp(diffci.noil6.12[exp.cols])
  }
...
results.table <- function(outcome = c('adl', 'faq', 'rbans', 'trailb', 'ltmort')){
  if(outcome %in% c('adl', 'faq')){
    results.data <- nb.results.data(outcome)
  } else if(outcome %in% c('rbans', 'trailb', 'ltmort')){
...
dchome.pred.il6 <- Predict(dchome.il6, nutric.il6 = NA, fun = exp)

options(datadist = 'dd.noil6.survivors')
dchome.sum.noil6 <- summary(dchome.noil6)
dchome.pred.noil6 <- Predict(dchome.noil6, nutric.noil6 = NA, fun = exp)
...
<<printdchomemodels, results = 'asis', fig.cap = 'NUTRIC Score at ICU Admission vs Discharge to Location Other than Home'>>=
# latex(dchome.nutric.table[,c('time.f', 'comp.str.il6', 'peci.str.il6', 'pval.str.il6',
...
## -- Run models -----------------------------------------------------------------------------------
lt.mort.rh <- "rcs(edu, 3) + frailty.num + sex.pp + rcs(bmi, 3) + iqcode.score.e + adl.e + rcs(faq.e, 3) + rcs(mean.modsofa.icu, 3) + rcs(icudays.sevseptic.s, 3) + rcs(del.s.imp, 3) + rcs(coma.s.imp, 3) + rcs(vent.los.tot.s, 3)"
...
follow. Because all of the long-term outcome models have the same amounts of missingness,
missingness for each cohort is presented in Table \ref{table:ltmissing}.
...
of NUTRIC score with FAQ score and adjusted differences in RBANS scores between the 75th and the
25th percentiles of our population's NUTRIC scores.

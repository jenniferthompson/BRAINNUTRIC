...
yaxis.labels <-
  data.frame(outcome = c('adl', 'faq', 'rbans', 'trailb', 'pcs', 'mcs', 'ltmort', 'dchome'),
             ylabel = c('Katz ADL Score', 'FAQ Score', 'RBANS Global Score', 'Trails B Score',
                        'SF-36 PCS', 'SF-36 MCS',
                        'Hazard of Death', 'Probability of\nDischarge to Non-Home'))
...
Results for the associations between NUTRIC scores and mortality after hospital discharge follow.
All models are Cox proportional hazards models using multiple imputation.
...
## -- Calculate HRs + CIs, predicted values --------------------------------------------------------
options(datadist = 'dd.il6.survivors')
ltmort.sum.il6.3 <- summary(ltmort.il6.3)
ltmort.pred.il6.3 <- Predict(ltmort.il6.3, nutric.il6 = NA, fun = exp)
ltmort.sum.il6.12 <- summary(ltmort.il6.12)
ltmort.pred.il6.12 <- Predict(ltmort.il6.12, nutric.il6 = NA, fun = exp)

options(datadist = 'dd.noil6.survivors')
ltmort.sum.noil6.3 <- summary(ltmort.noil6.3)
ltmort.pred.noil6.3 <- Predict(ltmort.noil6.3, nutric.noil6 = NA, fun = exp)
ltmort.sum.noil6.12 <- summary(ltmort.noil6.12)
ltmort.pred.noil6.12 <- Predict(ltmort.noil6.12, nutric.noil6 = NA, fun = exp)

ltmort.nutric.plot <- create.nutric.plot('ltmort')

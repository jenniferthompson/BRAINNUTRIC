...
create.nutric.plot.nb <- function(outcome = c('adl', 'faq')){
  ...
  ggplot(data = preddata, aes(x = xvalue, y = count.pe, ymin = count.lcl, ymax = count.ucl)) +
    ...
    scale_y_continuous(name = paste('Adjusted', outcome.ylab, 'Score')) +
...
nb.missing.table <- function(outcome = c('adl', 'faq')){
  nmis.3m.il6 <- get(paste0(outcome, '.il6.3'))$nmis
  nmis.3m.noil6 <- get(paste0(outcome, '.noil6.3'))$nmis
  nmis.12m.il6 <- get(paste0(outcome, '.il6.12'))$nmis
  nmis.12m.noil6 <- get(paste0(outcome, '.noil6.12'))$nmis
  ...
  miss.data[rownames(miss.data) %in% c(lt.covars, 'nutric', paste0(outcome, '.lt')), 9:12]
}

...
allmort.bynutric <- survfit(allmort.Surv ~ nutric.noil6, data = brain.oneobs)
...
ggsurv(allmort.bynutric, cens.col = '#BA6E00', cens.shape = NA) +
  scale_colour_manual(name = 'Modified NUTRIC,\nICU Admission', values = nutric.colors) +
...
## -- Create poster version of discharge to home plot, using modified IL6 only ---------------------
dchome.preddata <- create.rms.preddata('dchome')
dchome.plot.poster <- ggplot(data = subset(dchome.preddata, version.f == 'Without IL-6'),
                             aes(x = xvalue, y = yhat, ymin = lower, ymax = upper)) +
  scale_x_continuous(breaks = seq(0, max(dchome.preddata$xvalue), 1),
                     name = 'Modified NUTRIC Score at ICU Admission') +
  scale_y_continuous(limits = c(0, 1), name = 'Adjusted Probability of Discharge to Non-Home') +
  geom_pointrange(colour = "#003D79") +
  theme_bw()

pdf(file = 'modnutric_dchome_poster.pdf', width = 7, height = 7)
dchome.plot.poster
dev.off()

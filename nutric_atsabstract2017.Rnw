...
fu.outcomes <- brain.fu %>%
  ...
  gather(key = test, value = score, rbans.global.score:adl.totscore) %>%
...
desc.vars <- c('age.enroll', 'sex.pp', 'race.pp', 'edu', 'charlson.score', 'num.comorbid',
               ...
               'num.apache', 'sofa', 'mean.benz.icu', 'mean.op.new.icu', 'mean.prop.icu',
               'mean.dex.icu', 'mean.hal.icu', 'ever.vent.s', 'vent.los.tot.eo', 'ever.del.s.imp',
               'del.s.imp.eo', 'ever.coma.s.imp', 'coma.s.imp.eo', 'died.inhosp', 'hospdis.loc',
               'rbans.global.3', 'rbans.global.12', 'trail.b.3', 'trail.b.12', 'adl.3', 'adl.12',
               'faq.3', 'faq.12')

desc.data.all <- brain.oneobs[brain.oneobs$cohort.allpts, c('id', desc.vars)]
desc.data.all$cohort <- 1

desc.data.surv <- brain.oneobs[brain.oneobs$cohort.survivors, c('id', desc.vars)]
desc.data.surv$cohort <- 2

desc.data.lt <- brain.oneobs[brain.oneobs$cohort.lt, c('id', desc.vars)]
...
latex(summaryM(formula(paste(paste(desc.vars, collapse = ' + '), '~ cohort')),
      ...
      file = '',
      digits = 2,
      caption = 'Descriptive Statistics for Each Cohort',
      caption.lot = 'Descriptive Statistics for Each Cohort',
      label = 'table:table1',
      prn = FALSE,
      what = '%',
      npct = 'both',

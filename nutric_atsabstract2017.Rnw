...
each outcome. All hypothesis testing is done using the Wald method.
...
create.nb.preddata <- function(test = c('adl', 'faq')){
  mod.il6.3 <- get(paste0(test, '.il6.3'))
  mod.noil6.3 <- get(paste0(test, '.noil6.3'))
  mod.il6.12 <- get(paste0(test, '.il6.12'))
  mod.noil6.12 <- get(paste0(test, '.noil6.12'))
...
create.nutric.plot.nb <- function(test = c('adl', 'faq')){
  ## Get data set of predicted values for each time/version combination
  preddata <- create.nb.preddata(test)
  ...
  ggplot(data = preddata, aes(x = xvalue, y = count.pe, ymin = count.lcl, ymax = count.ucl)) +
    facet_grid(time.f ~ version.f) +
    scale_x_continuous(breaks = seq(0, max(preddata$xvalue), 1),
                       name = 'NUTRIC Score at ICU Admission') +
    scale_y_continuous(name = paste('Adjusted', test.ylab, 'Score')) +
...
## -- Step 6: Function to create data set of results for an outcome, both times & NUTRIC vsns ------
nb.results.data <- function(test = c('adl', 'faq')){
  mod.nonutric.il6.3 <- get(paste0(test, '.nonutric.il6.3'))
  mod.il6.3 <- get(paste0(test, '.il6.3'))
  mod.nonutric.noil6.3 <- get(paste0(test, '.nonutric.noil6.3'))
  mod.noil6.3 <- get(paste0(test, '.noil6.3'))
  mod.nonutric.il6.12 <- get(paste0(test, '.nonutric.il6.12'))
  mod.il6.12 <- get(paste0(test, '.il6.12'))
  mod.nonutric.noil6.12 <- get(paste0(test, '.nonutric.noil6.12'))
  mod.noil6.12 <- get(paste0(test, '.noil6.12'))
  ...
  p.noil6.12 <-
    pool.compare(mod.noil6.12, mod.nonutric.noil6.12, data = mice.noil6.12, method = use.method)$pvalue
  ...
  irrp.data$version.f <- factor(c(1, 2, 1, 2), levels = 1:2, labels = c('With IL-6', 'Without IL-6'))
  
  irrp.data$comp.str <- with(irrp.data, paste(comp.val, 'vs.', ref.val))
  irrp.data$irrci.str <- with(irrp.data, paste0(rndformat(pointest), ' (',
                                                rndformat(lcl), ', ',
                                                rndformat(ucl), ')'))
...
## -- Step 7: Create a data set for printing results from nb.results.data() in a table -------------
nb.results.table <- function(test = c('adl', 'faq')){
  irrp.data <- nb.results.data(test) %>%
    dplyr::select(time.f, version.f, comp.str, irrci.str, pval.str) %>%
  ...
  irrp.data[,c('time.f',
               'comp.str.il6', 'irrci.str.il6', 'pval.str.il6',
               'comp.str.noil6', 'irrci.str.noil6', 'pval.str.noil6')]
}

## -- Function to create a table showing missingness for each covariate in a model -----------------
nb.missing.table <- function(test = c('adl', 'faq')){
  nmis.3m.il6 <- get(paste0(test, '.il6.3'))$nmis
  nmis.3m.noil6 <- get(paste0(test, '.noil6.3'))$nmis
  nmis.12m.il6 <- get(paste0(test, '.il6.12'))$nmis
  nmis.12m.noil6 <- get(paste0(test, '.noil6.12'))$nmis
  names(nmis.3m.il6) <- gsub('\\.[no]*il6', '', gsub('\\.3', '.lt', names(nmis.3m.il6)))
  names(nmis.3m.noil6) <- gsub('\\.[no]*il6', '', gsub('\\.3', '.lt', names(nmis.3m.noil6)))
  names(nmis.12m.il6) <- gsub('\\.[no]*il6', '', gsub('\\.12', '.lt', names(nmis.12m.il6)))
  names(nmis.12m.noil6) <- gsub('\\.[no]*il6', '', gsub('\\.12', '.lt', names(nmis.12m.noil6)))
  
  pctmis.3m.il6 <- round((nmis.3m.il6 / n.3m.il6)*100)
  pctmis.3m.noil6 <- round((nmis.3m.noil6 / n.3m)*100)
  pctmis.12m.il6 <- round((nmis.12m.il6 / n.12m.il6)*100)
  pctmis.12m.noil6 <- round((nmis.12m.noil6 / n.12m)*100)
  
  miss.data <- data.frame(cbind(nmis.3m.il6, nmis.3m.noil6, nmis.12m.il6, nmis.12m.noil6,
                                pctmis.3m.il6, pctmis.3m.noil6, pctmis.12m.il6, pctmis.12m.noil6))
  miss.data$npct.3m.il6 <- with(miss.data, paste0(pctmis.3m.il6, '\\% (', nmis.3m.il6, ')'))
  miss.data$npct.3m.noil6 <- with(miss.data, paste0(pctmis.3m.noil6, '\\% (', nmis.3m.noil6, ')'))
  miss.data$npct.12m.il6 <- with(miss.data, paste0(pctmis.12m.il6, '\\% (', nmis.12m.il6, ')'))
  miss.data$npct.12m.noil6 <- with(miss.data, paste0(pctmis.12m.noil6, '\\% (', nmis.12m.noil6, ')'))
  
  miss.data[rownames(miss.data) %in% c(lt.covars, 'nutric', paste0(test, '.lt')), 9:12]
}
...
\clearpage
\section{NUTRIC Scores vs ADLs (Katz ADL)}
<<adlmodels>>=
## -- 3-month ADL ----------------------------------------------------------------------------------
...
adl.nutric.plot <- create.nutric.plot.nb('adl')
adl.nutric.table <- nb.results.table('adl')
...
latex(adl.nutric.table[,c('time.f', 'comp.str.il6',
                          'irrci.str.il6', 'pval.str.il6',
                          'irrci.str.noil6', 'pval.str.noil6')],
      file = '',
      where = '!h',
      caption = 'NUTRIC Score at ICU Admission vs Katz ADL Scores at 3 \\& 12 Months',
      label = 'table:adlresults',
      rowname = NULL,
      colheads = c('Followup Time', 'NUTRIC Score Comparison*', rep(c('IRR (95\\% CI)', 'P-Value'), 2)),
...
\clearpage
\section{NUTRIC Scores vs IADLs (Functional Activities Questionnaire)}
<<faqmodels>>=
## -- 3-month FAQ ----------------------------------------------------------------------------------
## Fit models without NUTRIC score to use in pool.compare()
faq.nonutric.il6.3 <-
...
## -- Calculate and plot predicted values ----------------------------------------------------------
faq.nutric.plot <- create.nutric.plot.nb('faq')
faq.nutric.table <- nb.results.table('faq')
...
latex(faq.nutric.table[,c('time.f', 'comp.str.il6',
                          'irrci.str.il6', 'pval.str.il6',
                          'irrci.str.noil6', 'pval.str.noil6')],
      file = '',
      where = '!h',

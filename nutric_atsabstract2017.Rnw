...
## Cohort for in-hospital mortality = >=1 day with mental status + no missing baseline covariates
brain.oneobs$cohort.inhosp <-
  with(brain.oneobs, cohort.allpts & !(no.days.mental | miss.base.covar))
n.inhosp <- sum(brain.oneobs$cohort.inhosp)
...
brain.oneobs$cohort.inhosp.il6 <- with(brain.oneobs, cohort.inhosp & !is.na(nutric.il6))

n.allpts.il6 <- sum(brain.oneobs$cohort.allpts.il6)
n.inhosp.il6 <- sum(brain.oneobs$cohort.inhosp.il6)
...
cohort.ns <- data.frame(cohort = c('Enrolled patients',
                                   ...
                                   'Long-term cognitive, disability, HRQOL',
                                   '~~~3 months',
                                   '~~~12 months'),
                        n.il6 = c(n.allpts.il6, n.inhosp.il6, n.survivors.il6,
                                  n.lt.il6, n.3m.il6, n.12m.il6),
                        n.noil6 = c(n.allpts, n.inhosp, n.survivors, n.lt, n.3m, n.12m))
...
be included in models using the full vs. the modified NUTRIC score. Patients are included in long-term cognitive, disability, and HRQOL analyses if they have at least partial outcomes data at that time point (ie, at least one of MMSE, Trailmaking A or B, any RBANS domain, FAQ, ADL, and/or SF-36 PCS or MCS).
...
\clearpage
Figures \ref{fig:km365all} and \ref{fig:km365survivors} show Kaplan-Meier curves representing mortality over the year following enrollment for all patients and for hospital
survivors only, respectively. \textbf{Please interpret these with caution}; because very few
...
ltmort.Surv <- with(subset(brain.oneobs, cohort.lt), {
  Surv(time = days.deathlast.365, event = as.numeric(died.brain.365)) })
ltmort.bynutric <- survfit(ltmort.Surv ~ nutric.il6, data = subset(brain.oneobs, cohort.lt))
...
ggsurv(allmort.bynutric, cens.col = '#BA6E00', cens.shape = NA) +
  ...
  ggtitle('All BRAIN Patients') +
  theme_bw()

@

<<km365survivors, results='asis', fig.cap='Kaplan-Meier Curve, Mortality among Hospital Survivors up to 365 Days after Enrollment', fig.height = 4>>=
## Patients who survived hospital stay
ggsurv(ltmort.bynutric, cens.col = '#BA6E00', cens.shape = NA) +
  scale_colour_manual(name = 'NUTRIC,\nICU Admission', values = nutric.colors) +
  scale_x_continuous(name = 'Days Since Enrollment', breaks = c(0, 30, 90, 180, 365)) +
  scale_y_continuous('Probability Alive') +
  guides(linetype = FALSE) +
  ggtitle('BRAIN Hospital Survivors') +
  theme_bw()
...
To avoid bias as much as possible, models for all long-term mortality, cognitive, disability and
HRQOL outcomes are done using multiple imputation. All patients with at least partial outcomes data
at that time point are included in the cohort; we use multiple imputation (specifically, predictive
mean matching) to impute both missing covariates and missing outcomes for all of these patients. Due
to the complexity of imputing both baseline and time-varying covariates in time-varying Cox models,
and the low proportion of missingness
(\Sexpr{round((sum(!(brain.oneobs$cohort.inhosp)) / nrow(brain.oneobs))*100)}\% of patients total)
and therefore low likelihood of bias, only patients with complete data are included in in-hospital
mortality models. Details of missing data are reported for each model in section \ref{section:missingness}.

All models include the following covariates, with changes as noted for in-hospital mortality; due to
the potential for immortal time bias with this outcome, we will use time-varying Cox models which
incorporate daily variables rather than summary variables. Most continuous covariates (noted below)
were allowed to have a nonlinear relationship with the outcomes using restricted cubic splines;
other continuous covariates were forced to have a linear relationship, due both to limited sample
size and to lack of variability in those covariates (eg, over 75\% of baseline ADL values were 0).
...
\item In-hospital characteristics
  ...
  \item Duration of delirium (nonlinear) (\emph{in-hospital death: daily mental status})
  \item Duration of coma (nonlinear) (\emph{in-hospital death: daily mental status})
...
death.inhosp.vars.tv <- c('daily.sofa.mod.locf', 'sevsepsis.imp', 'mental.stat.imp', 'on.vent.l24')

death.inhosp.orgdata <- brain.daily[,c('id', 'study.day', death.inhosp.vars.tv)] %>%
  left_join(dplyr::select(brain.oneobs, id, cohort.inhosp), by = 'id') %>%
  filter(cohort.inhosp)

death.inhosp.data <- timevarying.data(org.data = death.inhosp.orgdata,
...
death.inhosp.vars.base <-
  c('edu', 'frailty.num', 'sex.pp', 'bmi', 'iqcode.score.e', 'adl.e', 'faq.e')

death.inhosp.noil6.data <- left_join(death.inhosp.data,
                                     brain.oneobs[,c('id', death.inhosp.vars.base, 'nutric.noil6')],
                                     by = 'id') %>%
  filter(complete.cases(.))

death.inhosp.il6.data <-
  left_join(subset(death.inhosp.data, id %in% subset(brain.oneobs, cohort.inhosp.il6)$id),
            brain.oneobs[,c('id', death.inhosp.vars.base, 'nutric.il6')],
            by = 'id') %>%
  filter(complete.cases(.))

## -- Long-term mortality and discharge location (hospital survivor cohorts) -----------------------
...
rms.results.data <- function(outcome = c('rbans', 'trailb', 'pcs', 'mcs', 'ltmort', 'dchome',
                                         'inhospmort')){
  if(outcome %in% c('dchome', 'inhospmort')){
    mod.il6 <- get(paste0(outcome, '.il6'))
    mod.noil6 <- get(paste0(outcome, '.noil6'))
  ...
  if(outcome %in% c('ltmort', 'dchome', 'inhospmort')){
    exp.cols <- c(4, 6, 7)
  ...
  if(outcome %in% c('inhospmort', 'dchome')){
    if(outcome == 'dchome'){
      tlevel <- 1
      tlabel <- 'Discharge'
    } else{
    ...
    diffcip.data$time.f <- factor(rep(tlevel, 2), levels = tlevel, labels = tlabel)
...
nutric.ref <- floor(median(subset(brain.oneobs, cohort.inhosp.il6)$nutric.il6, na.rm = TRUE))
...
nutric.il6.pct.inhosp <- get.nutric.pcts('cohort.inhosp.il6', 'nutric.il6')
nutric.noil6.pct.inhosp <- get.nutric.pcts('cohort.inhosp', 'nutric.noil6')
...
Table \ref{table:inhospmissing} presents details on patients excluded from the complete case analyses used for in-hospital mortality.
...
missing.inhosp.ns <-
  matrix(c(n.allpts,
           ...
           sum(brain.oneobs$cohort.inhosp),
           ...
           nrow(subset(brain.oneobs,
                       cohort.allpts & !miss.base.covar & !no.days.mental & !is.na(nutric.noil6)))),
         ncol = 1)
rownames(missing.inhosp.ns) <- c('Enrolled in BRAIN',
                                 ...
                                 'Included in model with full NUTRIC',
                                 'Missing NUTRIC without IL-6',
                                 'Included in model with modified NUTRIC')
         
@

<<printinhospmissing, results = 'asis'>>=
latex(missing.inhosp.ns, file = '',
      title = '',
      where = '!h',
      caption = 'Missing Data for In-Hospital Mortality Models',
      caption.lot = 'Missing Data for In-Hospital Mortality Models',
      label = 'table:inhospmissing',
...
respectively, hospital survivors (used for discharge location and long-term mortality models) and
...
latex(missing.survivors.df[,2:3], file = '',
      title = '',
      where = '!h',
      caption = 'Missing Data for Discharge Location and Long-Term Mortality Models',
      caption.lot = 'Missing Data for Discharge, Long-Term Mortality Models',
...
\clearpage
\section{NUTRIC vs In-Hospital Mortality}
Results follow for the associations between NUTRIC scores and in-hospital mortality during the 30-day BRAIN study period. Both models are Cox proportional hazards models using time-dependent covariates for daily modified SOFA, severe sepsis, mental status, and ventilator status. Only patients (and patient-days) with complete data are included in this complete case analysis; please see Section \ref{section:missingness} for details on missingness in this cohort.

<<inhospmortmodels>>=
## -- Model setup ----------------------------------------------------------------------------------
Surv.il6.inhosp <- with(death.inhosp.il6.data, {
  Surv(time = start.day, time2 = stop.day, event = (died == 'Died at end of interval')) })
Surv.noil6.inhosp <- with(death.inhosp.noil6.data, {
  Surv(time = start.day, time2 = stop.day, event = (died == 'Died at end of interval')) })

inhospmort.covar.rh <- 'rcs(edu, 3) + frailty.num + sex.pp + rcs(bmi, 3) + iqcode.score.e + adl.e + rcs(faq.e, 3) + rcs(daily.sofa.mod.locf, 3) + sevsepsis.imp + mental.stat.imp + on.vent.l24'

inhospmort.il6.rh <- paste(inhospmort.covar.rh, '+ rcs(nutric.il6, 3)')
inhospmort.noil6.rh <- paste(inhospmort.covar.rh, '+ rcs(nutric.noil6, 3)')
...
dd.inhosp.il6 <- datadist(death.inhosp.il6.data)
dd.inhosp.noil6 <- datadist(death.inhosp.noil6.data)

## -- Run models -----------------------------------------------------------------------------------
inhospmort.il6 <- cph(as.formula(paste('Surv.il6.inhosp', inhospmort.il6.rh, sep = ' ~ ')),
                      x = TRUE, y = TRUE, data = death.inhosp.il6.data)

inhospmort.noil6 <- cph(as.formula(paste('Surv.noil6.inhosp', inhospmort.noil6.rh, sep = ' ~ ')),
                        x = TRUE, y = TRUE, data = death.inhosp.noil6.data)

## -- Calculate and plot HRs + CIs -----------------------------------------------------------------
## HRs for 75th vs 25th percentiles
options(datadist = 'dd.inhosp.il6')
inhospmort.sum.il6 <- summary(inhospmort.il6, nutric.il6 = nutric.il6.pct.inhosp)
inhospmort.hrs.il6 <- calc.nutric.hrs(inhospmort.il6, 'nutric.il6', reflevel = nutric.ref)
inhospmort.hrs.il6$version <- 1

options(datadist = 'dd.inhosp.noil6')
inhospmort.sum.noil6 <- summary(inhospmort.noil6, nutric.noil6 = nutric.noil6.pct.inhosp)
inhospmort.hrs.noil6 <- calc.nutric.hrs(inhospmort.noil6, 'nutric.noil6', reflevel = nutric.ref)
inhospmort.hrs.noil6$version <- 2

## Create results table
inhospmort.nutric.table <- results.table('inhospmort')

## Plot multiple HRs
inhospmort.hrs.all <-
  bind_rows(inhospmort.hrs.il6, inhospmort.hrs.noil6) %>%
  mutate(version.f = factor(version, levels = 1:2, labels = c('With IL-6', 'Without IL-6')))

inhospmort.nutric.plot <- ggplot(data = inhospmort.hrs.all, aes(x = xval, y = pointest)) +
  geom_hline(yintercept = 1, colour = 'grey90', size = 2) +
  facet_wrap(~ version.f, nrow = 1, scales = 'free_x') +
...
We see no meaningful association between NUTRIC score at baseline and in-hospital mortality up to 30 days.

Table \ref{table:inhospmortresults} shows p-values for the total association of NUTRIC score with
in-hospital mortality and hazard ratios comparing the 75th to the 25th percentiles of our population's NUTRIC scores. Figure \ref{fig:printinhospmortmodels} shows hazard ratios comparing each NUTRIC value to \Sexpr{nutric.ref}, our complete case cohort's median and thus representing an ``average"
patient. Figure \ref{fig:printinhospexplore} shows raw (unadjusted) times to in-hospital death or discharge by full NUTRIC score at ICU admission.
  
<<printinhospmortmodels, results = 'asis', fig.cap = 'NUTRIC Score at ICU Admission vs In-Hospital Mortality up to 30 Days', fig.height = 2.9>>=
latex(inhospmort.nutric.table[,c('comp.str.il6', 'peci.str.il6', 'pval.str.il6',
                                 'comp.str.noil6', 'peci.str.noil6', 'pval.str.noil6')],
      file = '',
      where = '!h',
      caption = 'NUTRIC Score at ICU Admission vs In-Hospital Mortality up to 30 Days',
      caption.lot = 'NUTRIC vs In-Hospital Mortality',
      label = 'table:inhospmortresults',
      rowname = NULL,
      colheads = rep(c('NUTRIC Score Comparison', 'HR (95\\% CI)', 'P-Value'), 2),
      col.just = rep(c('c', rep('r', 2)), 2),
      cgroup = c('NUTRIC with IL-6', 'NUTRIC without IL-6'),
      n.cgroup = c(3, 3))

inhospmort.nutric.plot

@

<<printinhospexplore, results = 'asis', fig.cap = 'Unadjusted Time to In-Hospital Death or Discharge by Modified NUTRIC', fig.height = 2.7>>=
ggplot(data = subset(brain.oneobs, cohort.inhosp.il6),
       aes(x = nutric.il6, y = days.deathlast.inhosp.s)) +
  geom_boxplot(aes(group = nutric.il6), colour = 'grey50', outlier.shape = NA, fill = NA) +
  geom_point(aes(colour = died.inhosp.s), position = position_jitter(width = 0.5), alpha = 0.5, size = 0.75) +
...
dchome.sum.noil6 <- summary(dchome.noil6, nutric.noil6 = nutric.noil6.pct.inhosp)
...
\clearpage
\section{NUTRIC vs Long-Term Mortality}
Results follow for the associations between NUTRIC scores and mortality after hospital discharge,
among hospital survivors only. All models are Cox proportional hazards models using multiple
imputation.

<<mortmodels>>=
## -- Create Surv objects --------------------------------------------------------------------------
Surv.il6.3m <- with(modeldata.il6.survivors, {
  Surv(time = days.deathlast.90, event = (died.brain.90 == 'Died during first 90 days')) })
Surv.noil6.3m <- with(modeldata.noil6.survivors, {
  Surv(time = days.deathlast.90, event = (died.brain.90 == 'Died during first 90 days')) })
Surv.il6.12m <- with(modeldata.il6.survivors, {
  Surv(time = days.deathlast.365, event = (died.brain.365 == 'Died during first 365 days')) })
Surv.noil6.12m <- with(modeldata.noil6.survivors, {
  Surv(time = days.deathlast.365, event = (died.brain.365 == 'Died during first 365 days')) })

## -- Run models -----------------------------------------------------------------------------------
ltmort.il6.3 <- fit.mult.impute(as.formula(paste('Surv.il6.3m', lt.il6.rh, sep = ' ~ ')),
                                fitter = cph,
                                x = TRUE, y = TRUE,
                                xtrans = mice.il6.survivors,
                                data = modeldata.il6.survivors)

ltmort.noil6.3 <- fit.mult.impute(as.formula(paste('Surv.noil6.3m', lt.noil6.rh, sep = ' ~ ')),
                                  fitter = cph,
                                  x = TRUE, y = TRUE,
                                  xtrans = mice.noil6.survivors,
                                  data = modeldata.noil6.survivors)

ltmort.il6.12 <- fit.mult.impute(as.formula(paste('Surv.il6.12m', lt.il6.rh, sep = ' ~ ')),
                                 fitter = cph,
                                 x = TRUE, y = TRUE,
                                 xtrans = mice.il6.survivors,
                                 data = modeldata.il6.survivors)

ltmort.noil6.12 <- fit.mult.impute(as.formula(paste('Surv.noil6.12m', lt.noil6.rh, sep = ' ~ ')),
                                   fitter = cph,
                                   x = TRUE, y = TRUE,
                                   xtrans = mice.noil6.survivors,
                                   data = modeldata.noil6.survivors)
...
## HRs for 75th vs 25th percentiles
ltmort.sum.il6.3 <- summary(ltmort.il6.3, nutric.il6 = nutric.il6.pct.survivors)
ltmort.sum.il6.12 <- summary(ltmort.il6.12, nutric.il6 = nutric.il6.pct.survivors)

## HRs vs median
ltmort.hrs.il6.3 <- calc.nutric.hrs(ltmort.il6.3, 'nutric.il6', reflevel = nutric.ref)
ltmort.hrs.il6.3$time <- 1; ltmort.hrs.il6.3$version <- 1
ltmort.hrs.il6.12 <- calc.nutric.hrs(ltmort.il6.12, 'nutric.il6', reflevel = nutric.ref)
ltmort.hrs.il6.12$time <- 2; ltmort.hrs.il6.12$version <- 1

## Without IL-6
options(datadist = 'dd.noil6.survivors')

## HRs for 75th vs 25th percentiles
ltmort.sum.noil6.3 <- summary(ltmort.noil6.3, nutric.noil6 = nutric.noil6.pct.survivors)
ltmort.sum.noil6.12 <- summary(ltmort.noil6.12, nutric.noil6 = nutric.noil6.pct.survivors)

ltmort.hrs.noil6.3 <- calc.nutric.hrs(ltmort.noil6.3, 'nutric.noil6', reflevel = nutric.ref)
ltmort.hrs.noil6.3$time <- 1; ltmort.hrs.noil6.3$version <- 2
ltmort.hrs.noil6.12 <- calc.nutric.hrs(ltmort.noil6.12, 'nutric.noil6', reflevel = nutric.ref)
ltmort.hrs.noil6.12$time <- 2; ltmort.hrs.noil6.12$version <- 2

## Create results table and plot
ltmort.nutric.table <- results.table('ltmort')

ltmort.hrs.all <-
  bind_rows(ltmort.hrs.il6.3, ltmort.hrs.il6.12, ltmort.hrs.noil6.3, ltmort.hrs.noil6.12) %>%
  mutate(time.f = factor(time, levels = 1:2, labels = c('3 Months', '12 Months')),
         version.f = factor(version, levels = 1:2, labels = c('With IL-6', 'Without IL-6')))

ltmort.nutric.plot <- ggplot(data = ltmort.hrs.all, aes(x = xval, y = pointest)) +
  geom_hline(yintercept = 1, colour = 'grey90', size = 2) +
...
# ## Test with survest()
# survest.df <- data.frame(nutric.noil6 = 0:9,
#                          edu = rep(dd.noil6.3$limits$edu[2], 10),
#                          frailty.num = rep(dd.noil6.3$limits$frailty.num[2], 10),
#                          sex.pp = rep(dd.noil6.3$limits$sex.pp[2], 10),
#                          bmi = rep(dd.noil6.3$limits$bmi[2], 10),
#                          iqcode.score.e = rep(dd.noil6.3$limits$iqcode.score.e[2], 10),
#                          adl.e = rep(dd.noil6.3$limits$adl.e[2], 10),
#                          faq.e = rep(dd.noil6.3$limits$faq.e[2], 10),
#                          mean.modsofa.icu = rep(dd.noil6.3$limits$mean.modsofa.icu[2], 10),
#                          icudays.sevseptic.s = rep(dd.noil6.3$limits$icudays.sevseptic.s[2], 10),
#                          del.s.imp = rep(dd.noil6.3$limits$del.s.imp[2], 10),
#                          coma.s.imp = rep(dd.noil6.3$limits$coma.s.imp[2], 10),
#                          vent.los.tot.s = rep(dd.noil6.3$limits$vent.los.tot.s[2], 10))
# 
# survest.ltmort.noil6.3 <- survest(ltmort.noil6.3, newdata = survest.df, times = 90)
# with(survest.ltmort.noil6.3, cbind(surv, lower, upper))
# Predict(ltmort.noil6.3, nutric.noil6 = NA, time = 90)[,c('yhat', 'lower', 'upper')]
# 
# survest.ltmort.noil6.12 <- survest(ltmort.noil6.12, newdata = survest.df[1,], times = c(90, 180, 350))
# 
# survest(ltmort.noil6.12, newdata = survest.df[1,], times = c(365))

@

We see no meaningful association between NUTRIC score at baseline and mortality at either 3 or 12
months after hospital discharge.

Table \ref{table:ltmortresults} shows p-values for the total association of NUTRIC score with
long-term mortality and hazard ratios comparing the 75th to the 25th percentiles of our population's
NUTRIC scores. Figure \ref{fig:printltmortmodels} shows hazard ratios comparing each NUTRIC value to
\Sexpr{nutric.ref}, our cohort of survivors' median.
  
<<printltmortmodels, results = 'asis', fig.cap = 'NUTRIC Score at ICU Admission vs Mortality at 3 \\& 12 Months'>>=
latex(ltmort.nutric.table[,c('time.f', 'comp.str.il6', 'peci.str.il6', 'pval.str.il6',
                             'peci.str.noil6', 'pval.str.noil6')],
      file = '',
      where = '!h',
      caption = 'NUTRIC Score at ICU Admission vs Mortality at 3 \\& 12 Months',
      caption.lot = 'NUTRIC vs 3- and 12-Month Mortality',
      label = 'table:ltmortresults',
      rowname = NULL,
      colheads = c('Followup Time', 'NUTRIC Score Comparison*',
                   rep(c('HR (95\\% CI)', 'P-Value'), 2)),

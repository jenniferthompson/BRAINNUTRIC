...
Results for the associations between NUTRIC scores and mortality at three time points follow. All
models are Cox proportional hazards models using multiple imputation; missingness for both
in-hospital (all patients) and long-term mortality (hospital survivors) models are shown in Table \ref{table:deathmissing}.

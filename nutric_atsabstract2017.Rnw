...
(\Sexpr{round((sum(!(brain.oneobs$cohort.mort)) / nrow(brain.oneobs))*100)}\% of patients total)
...
death.inhosp.il6.data <-
  left_join(subset(death.inhosp.data, id %in% subset(brain.oneobs, cohort.mort.il6)$id),
...
nutric.il6.pct.inhosp <- get.nutric.pcts('cohort.mort.il6', 'nutric.il6')
nutric.noil6.pct.inhosp <- get.nutric.pcts('cohort.mort', 'nutric.noil6')
